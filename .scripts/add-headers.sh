...
# Script to add standard headers to all Swift files in the Garnish project
...
AUTHOR="${GARNISH_AUTHOR:-Garnish Contributors}"
COPYRIGHT_HOLDER="${GARNISH_COPYRIGHT:-Garnish Contributors}"
YEAR=$(date +%Y)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "Options:"
            echo "  --author NAME          Set the author name (default: Garnish Contributors)"
            echo "  --copyright HOLDER     Set the copyright holder (default: Garnish Contributors)"
            echo "  --year YEAR           Set the year (default: current year)"
            echo ""
            echo "You can also set environment variables:"
            echo "  GARNISH_AUTHOR         Default author name"
            echo "  GARNISH_COPYRIGHT      Default copyright holder"
            exit 0
            ;;
        *)
...
add_header() {
    ...
    if [ -f "$PROJECT_ROOT/.header-template" ]; then
        ...
        # Fallback to inline template - detect module from path
